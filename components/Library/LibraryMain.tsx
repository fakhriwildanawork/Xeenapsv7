        ...
        <div className="relative">
          ...
          {showSortMenu && (
            <div className="absolute left-0 mt-2 w-52 bg-white rounded-2xl shadow-2xl border border-gray-100 z-[60] p-2 animate-in fade-in zoom-in-95">
              ...
              {['title', 'author', 'publisher', 'year', 'category', 'topic', 'subTopic', 'createdAt'].map((k) => (
            ...
            <tbody className="divide-y divide-gray-50">
              {isInternalLoading ? <TableSkeletonRows count={5} /> : serverItems.length === 0 ? <tr><td colSpan={tableColumns.length + 1} className="px-6 py-24 text-center"><div className="flex flex-col items-center justify-center space-y-2"><div className="p-4 bg-gray-50 rounded-full"><PlusIcon className="w-8 h-8 text-gray-300" /></div><p className="text-sm font-bold text-gray-400 uppercase tracking-widest">No Collection Found</p></div></td></tr> : serverItems.map((item) => (
                <StandardTr key={item.id} className="cursor-pointer" onClick={() => setSelectedItem(item)}>
                  ...
                  <StandardTd isActiveSort={sortConfig.key === 'author'}><ElegantTooltip text={item.authors?.join(', ') || ''}><div className="text-xs text-gray-600 italic text-center w-full block overflow-hidden line-clamp-2">{item.authors?.join(', ') || '-'}</div></ElegantTooltip></StandardTd>
                  <StandardTd isActiveSort={sortConfig.key === 'publisher'}><ElegantTooltip text={item.publisher}><div className="text-xs text-gray-600 text-center w-full block overflow-hidden line-clamp-2">{item.publisher || '-'}</div></ElegantTooltip></StandardTd>
                  <StandardTd isActiveSort={sortConfig.key === 'year'} className="text-xs text-gray-600 font-mono text-center">{item.year || '-'}</StandardTd>
                  <StandardTd isActiveSort={sortConfig.key === 'category'} className="text-xs text-gray-600 text-center"><div className="line-clamp-2">{item.category || '-'}</div></StandardTd>
                  <StandardTd isActiveSort={sortConfig.key === 'topic'}><ElegantTooltip text={item.topic}><div className="text-xs text-gray-600 text-center w-full block overflow-hidden line-clamp-2">{item.topic || '-'}</div></ElegantTooltip></StandardTd>
                  <StandardTd isActiveSort={sortConfig.key === 'subTopic'}><ElegantTooltip text={item.subTopic}><div className="text-xs text-gray-600 text-center w-full block overflow-hidden line-clamp-2">{item.subTopic || '-'}</div></ElegantTooltip></StandardTd>
          ...
          <StandardGridContainer>
            {serverItems.map((item) => (
              <StandardItemCard key={item.id} isSelected={selectedIds.includes(item.id)} onClick={() => setSelectedItem(item)}>
                ...
                <div className="flex items-center gap-3 mb-2" onClick={(e) => e.stopPropagation()}><button onClick={() => toggleSelectItem(item.id)} className={`w-5 h-5 rounded-full border flex items-center justify-center transition-all ${selectedIds.includes(item.id) ? 'bg-[#004A74] border-[#004A74] text-white' : 'bg-white border-gray-200'}`}>{selectedIds.includes(item.id) && <CheckIcon className="w-3 h-3 stroke-[4]" />}</button><span className="text-[8px] font-black uppercase tracking-widest bg-[#004A74] text-white px-2 py-0.5 rounded-full line-clamp-2">{item.category || 'GENERAL'}</span></div>
                <div className="flex items-center justify-between"><span className="text-[10px] font-black uppercase tracking-widest text-[#004A74] opacity-80 line-clamp-2">{item.topic || 'NO TOPIC'}</span></div>
                <div className="mt-[-4px] mb-2"><span className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter line-clamp-2">{item.subTopic || 'No Sub Topic'}</span></div>
                <div className="flex items-start gap-2 mb-3"><div className="flex-1 min-w-0"><h3 className="text-sm font-bold text-[#004A74] line-clamp-2 leading-tight">{item.title}</h3></div></div>
                <p className="text-xs font-medium text-gray-500 italic line-clamp-2 mb-1">{item.authors?.join(', ') || 'Unknown Author'}</p>
                <p className="text-[11px] text-gray-400 truncate mb-4">{item.publisher || '-'}</p>
